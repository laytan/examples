...
main :: proc() {
	...
	on_recv :: proc(op: ^nbio.Operation) {
		if op.recv.err != nil {
			fmt.eprintfln("recv: %v", nbio.error_string(op.recv.err))
			return
		}

		fmt.eprintfln("received %M from %v", op.recv.received, nbio.endpoint_to_string(op.recv.source))

		nbio.send(op.recv.socket, op.recv.buf[:op.recv.received], on_sent, op.recv.source)
	}

	on_sent :: proc(op: ^nbio.Operation) {
		if op.send.err != nil {
			fmt.eprintfln("send: %v", nbio.error_string(op.send.err))
			return
		}

		fmt.eprintfln("sent %M to %v", op.send.sent, nbio.endpoint_to_string(op.send.endpoint))

		nbio.recv(op.send.socket, raw_data(op.send.buf)[:BUF_LEN], on_recv)
	}
